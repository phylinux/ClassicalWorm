FUNCTION cal_energy()
	...
	do ib=1, Nb(ir)
		...
		cal_energy = cal_energy + &
			0.5d0 * (Pbadd(curr,-1)+Pbadd(curr,1))
	end do
	cal_energy = -Jcp(ir)*cal_energy
...
FUNCTION cal_E2()
	...
	do ib=1, Nb(ir)
		curr = current(ir,ib)
		cal_E2 = cal_E2 + &
			0.25d0 * (Pbadd(curr,-2)+2.d0+Pbadd(curr,2))
		cal_E2 = cal_E2 - &
			0.25d0 * (Pbadd(curr,-1)+Pbadd(curr,1))**2
	end do
	cal_E2 = Jcp(ir)*Jcp(ir)*cal_E2
END FUNCTION cal_E2
